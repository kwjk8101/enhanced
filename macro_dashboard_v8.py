...
# RISK INDICATORS
# ─────────────────────────────────────────────────────────────────────────────
section("Risk & Activity Indicators")
...
# ─────────────────────────────────────────────────────────────────────────────
section("Sector Rotation Signals & ETF Performance")
...
# ─────────────────────────────────────────────────────────────────────────────
section("Upcoming Economic Releases")
desc("Key US data releases that typically move markets. "
     "<strong>High impact</strong> events can cause sharp intraday moves in equities, bonds and FX. "
     "Dates are approximate — always confirm on the official FRED / BLS / BEA calendars before trading.")

# Static calendar — manually updated each month to reflect actual scheduled dates.
# Based on official BLS, BEA, Fed, and ISM release calendars.
...
ECON_CALENDAR = [
    # label, approx day-of-month, impact (H/M), source
    ("CPI Report",                 15, "HIGH",   "BLS"),
    ("Core CPI",                   15, "HIGH",   "BLS"),
    ("Fed FOMC Meeting / Decision", 20, "HIGH",   "Federal Reserve"),
    ("Non-Farm Payrolls (NFP)",     5,  "HIGH",   "BLS"),
    ("GDP Advance Estimate",       27, "HIGH",   "BEA"),
    ("PCE Inflation",              28, "HIGH",   "BEA"),
    ("Retail Sales",               16, "MEDIUM", "Census Bureau"),
    ("Initial Jobless Claims",      4, "MEDIUM", "DOL (weekly)"),
    ("Consumer Confidence",        25, "MEDIUM", "Conference Board"),
    ("ISM Manufacturing PMI",       2, "MEDIUM", "ISM"),
    ("ISM Services PMI",            5, "MEDIUM", "ISM"),
    ("Housing Starts",             18, "MEDIUM", "Census Bureau"),
    ("Producer Price Index (PPI)", 14, "MEDIUM", "BLS"),
    ("FOMC Minutes",               22, "MEDIUM", "Federal Reserve"),
]

cal_rows = []
for label, day, impact, source in ECON_CALENDAR:
    try:
        max_day = _cal.monthrange(_yr, _mo)[1]
        rel_day = min(day, max_day)
        rel_date = date(_yr, _mo, rel_day)
        days_away = (rel_date - _today).days
        if days_away < 0:
            # Show next month
            nm = _mo % 12 + 1
            ny = _yr + (1 if _mo == 12 else 0)
            max_day2 = _cal.monthrange(ny, nm)[1]
            rel_date = date(ny, nm, min(day, max_day2))
            days_away = (rel_date - _today).days
        cal_rows.append((label, rel_date, days_away, impact, source))
    except Exception:
        pass

cal_rows.sort(key=lambda x: x[1])

IMPACT_CLR = {"HIGH": "#f87171", "MEDIUM": "#f59e0b"}
IMPACT_BG  = {"HIGH": "rgba(248,113,113,.13)", "MEDIUM": "rgba(245,158,11,.1)"}

# Display as a grid of cards — next 8 events
cal_html = '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;">'
for label, rel_date, days_away, impact, source in cal_rows[:8]:
    ic  = IMPACT_CLR.get(impact, "#94a3b8")
    ibg = IMPACT_BG.get(impact, "rgba(148,163,184,.08)")
    when = "Today" if days_away == 0 else f"In {days_away}d" if days_away > 0 else f"{abs(days_away)}d ago"
    cal_html += (
        f'<div style="background:#0f1e35;border:1px solid #1f3354;border-radius:10px;'
        f'padding:14px;border-top:2px solid {ic}">'
        f'<div style="font-family:{MONO};font-size:.63rem;font-weight:700;color:{ic};'
        f'text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px">{impact}</div>'
        f'<div style="font-size:.83rem;font-weight:600;color:#dde8f5;margin-bottom:4px">{label}</div>'
        f'<div style="font-family:{MONO};font-size:.72rem;color:#8ab4d8">'
        f'{rel_date.strftime("%d %b %Y")} &nbsp;·&nbsp; <span style="color:#22d3ee">{when}</span></div>'
        f'<div style="font-size:.68rem;color:#4a6080;margin-top:4px">{source}</div>'
        f'</div>'
    )
cal_html += '</div>'
st.markdown(cal_html, unsafe_allow_html=True)
st.caption("Dates are estimates based on typical release schedules. Confirm on bls.gov / bea.gov before trading.")
...
# HEATMAP + SCORECARD
# ─────────────────────────────────────────────────────────────────────────────
section("Sector Regime Heatmap & Macro Scorecard")
...
sgt_f=now_sgt()
st.markdown(
    f'<div style="display:flex;justify-content:space-between;padding-top:14px;'
    f'border-top:1px solid #1f3354;margin-top:0.5rem">'
    f'<span style="font-family:{MONO};font-size:.7rem;color:#4a6080">Data: FRED · yfinance · Last loaded: {sgt_f.strftime("%d %b %Y %H:%M")} SGT</span>'
    f'<span style="font-family:{MONO};font-size:.7rem;color:#4a6080">Educational use only — not financial advice</span>'
    f'<span style="font-family:{MONO};font-size:.7rem;color:#4a6080">MACRO/SIGNAL v7.0 · {sgt_f.year}</span>'
    f'</div>',unsafe_allow_html=True)
